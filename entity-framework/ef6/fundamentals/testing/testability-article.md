...
Иобжектсет @ no__t-0T @ no__t-1 соответствует требованиям для репозитория, так как он напоминает коллекцию объектов (через IEnumerable @ no__t-2T @ no__t-3) и предоставляет методы для добавления и удаления объектов из имитации коллекции. Методы Attach и Detach предоставляют дополнительные возможности API EF4. Чтобы использовать Иобжектсет @ no__t-0T @ no__t-1 в качестве интерфейса для репозиториев, нам требуется абстракция единиц работы для привязки репозиториев вместе.
...
Перевод Иобжектсет @ no__t-0T @ no__t-1 в жизнь — это так же просто, как вызов метода CreateObject объекта ObjectContext. В фоновом режиме платформа будет использовать метаданные, предоставленные в EDM, для создания конкретного объектного типа @ no__t-0T @ no__t-1. Мы будем возвращать интерфейс Иобжектсет @ no__t-0T @ no__t-1, так как он поможет сохранить пригодность для тестирования в клиентском коде.
...
Для хранения объектов Employee и табели в памяти также потребуется фиктивный Иобжектсет @ no__t-0T @ no__t-1. Мы можем предоставить единую реализацию, используя универсальные шаблоны.
...
Когда мы хотим заранее загрузить связанные сведения о сущности, требуется некоторый механизм для бизнес-логики (или в этом сценарии — логика действия контроллера), чтобы выразить свое желание в репозитории. Класс EF4 ObjectQuery @ no__t-0T @ no__t-1 определяет метод Include, указывающий связанные объекты для извлечения во время выполнения запроса. Помните, что EF4 ObjectContext предоставляет сущности через конкретный класс Objecting @ no__t-0T @ no__t-1, который наследует от ObjectQuery @ no__t-2T @ no__t-3.  Если мы использовали набор объектов @ no__t-0T @ no__t-1 в действии контроллера, мы можем написать следующий код, чтобы указать безотлагательную загрузку сведений о карте времени для каждого сотрудника.
...
Однако, поскольку мы пытаемся сделать наш код тестируемым, мы не предоставляем объект набора объектов @ no__t-0T @ no__t-1 извне класса реальных единиц работы. Вместо этого мы полагаетесь на интерфейс Иобжектсет @ no__t-0T @ no__t-1, который проще имитировать, но Иобжектсет @ no__t-2T @ no__t-3 не определяет метод Include. Прелесть LINQ заключается в том, что мы можем создать собственный оператор include.
...
Обратите внимание, что этот оператор include определен как метод расширения для IQueryable @ no__t-0T @ no__t-1 вместо Иобжектсет @ no__t-2T @ no__t-3. Это дает нам возможность использовать метод с более широким диапазоном возможных типов, включая IQueryable @ no__t-0T @ no__t-1, Иобжектсет @ no__t-2T @ no__t-3, ObjectQuery @ no__t-4T поддерживается @ no__t-5, и Object @ no__t-6T @ no__t-7. В случае, если базовая последовательность не является подлинной EF4 ObjectQuery @ no__t-0T @ no__t-1, то ничего не происходит, а оператор include — это No-Op. Если базовая последовательность является ObjectQuery @ no__t-1T @ no__t-2 (или *является* производной от ObjectQuery @ No__t-3T @ no__t-4), то EF4 будет видеть наше требование к дополнительным данным и СФОРМУЛИРОВАТЬ правильный SQL-запрос.
...
На этом этапе мы сделаем наш контроль над созданием репозиториев с помощью Иобжектсет @ no__t-0T @ no__t-1 и взглянем на абстракции, чтобы скрыть все знаки инфраструктуры сохраняемости.
...
Основное различие между этой единицей работы и единицей работы, созданным в последнем разделе, заключается в том, как эта единица работы не использует абстракции из платформы EF4 (отсутствует Иобжектсет @ no__t-0T @ no__t-1). Иобжектсет @ no__t-0T @ no__t-1 работает в качестве интерфейса репозитория, но предоставляемый им API может быть не вполне согласован с потребностями нашего приложения. В этом предстоящем подходе мы будем представлять репозитории с помощью пользовательской абстракции IRepository @ no__t-0T @ no__t-1.

Многие разработчики, которые следуют разработке на основе тестирования, проектирования на основе поведения и методологии, ориентированной на работу с доменами, предпочитают подход IRepository @ no__t-0T @ no__t-1 по нескольким причинам. Во-первых, интерфейс IRepository @ no__t-0T @ no__t-1 представляет уровень защиты от повреждений. Как описано в руководстве по созданию и Эванс в своей книге разработки, основанной на домене, уровень защиты от вредоносных программ удерживает ваш код домена от API-интерфейсов инфраструктуры, таких как API сохраняемости. Во-вторых, разработчики могут создавать в репозитории методы, соответствующие точным потребностям приложения (как было обнаружено при написании тестов). Например, нам часто приходится искать одну сущность, используя значение идентификатора, поэтому мы можем добавить метод Финдбид в интерфейс репозитория.  Наше определение IRepository @ no__t-0T @ no__t-1 будет выглядеть следующим образом.
...
Обратите внимание, что мы вернемся к использованию интерфейса IQueryable @ no__t-0T @ no__t-1 для предоставления коллекций сущностей. IQueryable @ no__t-0T @ no__t-1 позволяет деревьям выражений LINQ передаваться в поставщик EF4 и предоставлять поставщику целостное представление запроса. Вторым вариантом будет возврат IEnumerable @ no__t-0T @ no__t-1, что означает, что поставщик EF4 LINQ будет видеть только выражения, собранные внутри репозитория. Группирование, упорядочивание и проекция, выполненные за пределами репозитория, не будут содержаться в команде SQL, отправленной в базу данных, что может понизить производительность. С другой стороны, репозиторий, возвращающий только результаты IEnumerable @ no__t-0T @ no__t-1, никогда не поудивит вас с новой командой SQL. Оба подхода будут работать, и оба подхода останутся тестируемыми.

Очень просто предоставить единую реализацию интерфейса IRepository @ no__t-0T @ no__t-1, используя универсальные шаблоны и API-интерфейс EF4 ObjectContext.
...
Для создания экземпляра Live IRepository @ no__t-0T @ no__t-1 требуется EF4 ObjectContext, поэтому конкретная реализация единиц работы должна управлять созданием экземпляра.
...
Использование нашего пользовательского репозитория незначительно отличается от использования репозитория на основе Иобжектсет @ no__t-0T @ no__t-1. Вместо применения операторов LINQ непосредственно к свойству сначала необходимо вызвать один из методов репозитория, чтобы получить ссылку на IQueryable @ no__t-0T @ no__t-1.
...
Мы задаем MOQ для реализации IRepository @ no__t-0Employee @ no__t-1, и она создает ее динамически. Чтобы получить доступ к объекту, реализующему IRepository @ no__t-0Employee @ no__t-1, можно обратиться к свойству Object объекта макетирование @ no__t-2T @ no__t-3. Это внутренний объект, который мы можем передать в контроллеры, и они не будут уверены, что это тестовый двойной или реальный репозиторий. Мы можем вызывать методы для объекта точно так же, как мы бы вызывали методы для объекта с реальной реализацией.
...
В этом примере мы просим MOQ динамически создавать репозиторий, а затем запрограммирован репозиторий с ожиданием. Ожидание означает, что макет объекта возвращает новый объект Employee со значением идентификатора 5, когда кто-то вызывает метод Финдбид, передающий значение 5. Этот тест пройден, и нам не нужно было создавать полную реализацию для имитации IRepository @ no__t-0T @ no__t-1.
...
В этом документе мы рассмотрели несколько подходов к созданию тестируемого кода при использовании Entity Framework ADO.NET для сохраняемости данных. Мы можем использовать встроенные абстракции, такие как Иобжектсет @ no__t-0T @ no__t-1, или создавать собственные абстракции, такие как IRepository @ no__t-2T @ no__t-3.  В обоих случаях поддержка POCO в ADO.NET Entity Framework 4,0 позволяет потребителям этих абстракций оставаться постоянной игнорирующих и высоко тестироваться. Дополнительные функции EF4, такие как неявная неактивная Загрузка, позволяют бизнес-приложениям и коду службы приложений работать, не беспокоясь о деталях реляционного хранилища данных. Наконец, создаваемые абстракции просты в макете или подмышлении в модульных тестах, и мы можем использовать эти тестовые удвоения для быстрого выполнения, высокой изолированности и надежности тестов.
...
-   Блог по программированию данных "@no__t 0Walkthrough: Разработка на основе тестирования с помощью Entity Framework 4.0 @ no__t-0 ".
