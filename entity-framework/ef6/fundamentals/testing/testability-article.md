...
## <a name="what-is-testable-code"></a><span data-ttu-id="c3c7b-111">Что такое тестируемого кода?</span><span class="sxs-lookup"><span data-stu-id="c3c7b-111">What Is Testable Code?</span></span>
...
### <a name="the-qualities-of-testable-code"></a><span data-ttu-id="c3c7b-126">Качество тестируемого кода</span><span class="sxs-lookup"><span data-stu-id="c3c7b-126">The Qualities of Testable Code</span></span>
...
### <a name="the-unit-of-work-pattern"></a><span data-ttu-id="c3c7b-189">Шаблон единицы работы</span><span class="sxs-lookup"><span data-stu-id="c3c7b-189">The Unit of Work Pattern</span></span>
...
-   <span data-ttu-id="c3c7b-311">Создайте объект newEmployee для имитации входящих данных.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-311">Create a newEmployee object to simulate incoming data.</span></span>
...
<span data-ttu-id="c3c7b-357">При выполнении для реальных ObjectContext, код создает следующий один запрос.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-357">When run against a real ObjectContext, the code produces the following single query.</span></span> <span data-ttu-id="c3c7b-358">Запрос собирает достаточно информации из базы данных в один обращения к материализации объектов employee и полностью заполнить свойству их труда.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-358">The query gathers enough information from the database in one trip to materialize the employee objects and fully populate their TimeCards property.</span></span>
...
<span data-ttu-id="c3c7b-359">Хорошая новость в том, остается полностью тестируемый код внутри метода действия.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-359">The great news is the code inside the action method remains fully testable.</span></span> <span data-ttu-id="c3c7b-360">Не требуется предоставить дополнительные компоненты для наших fakes для поддержки оператор Include.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-360">We don’t need to provide any additional features for our fakes to support the Include operator.</span></span> <span data-ttu-id="c3c7b-361">Плохая новость заключается в что нам было необходимо использовать оператор Include внутри кода, мы хотели бы сохранить игнорирующих сохраняемость.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-361">The bad news is we had to use the Include operator inside of the code we wanted to keep persistence ignorant.</span></span> <span data-ttu-id="c3c7b-362">Это — отличный пример типа необходимых действиях, которые необходимо оценить при построении тестируемого кода.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-362">This is a prime example of the type of tradeoffs you’ll need to evaluate when building testable code.</span></span> <span data-ttu-id="c3c7b-363">Бывают случаи, когда необходимо разрешить сохраняемости проблемы утечки за пределами абстракции репозитория в соответствии с целевыми показателями производительности.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-363">There are times when you need to let persistence concerns leak outside the repository abstraction to meet performance goals.</span></span>

<span data-ttu-id="c3c7b-364">Альтернативой Безотложная загрузка является отложенной загрузки.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-364">The alternative to eager loading is lazy loading.</span></span> <span data-ttu-id="c3c7b-365">Отложенная загрузка означает, что мы делаем *не* требуется код нашей бизнес явно объявить о требование для связанных данных.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-365">Lazy loading means we do *not* need our business code to explicitly announce the requirement for associated data.</span></span> <span data-ttu-id="c3c7b-366">Вместо этого мы используем наш сущностей в приложении и ли дополнительные данные, необходимые Entity Framework будет загружать данные по запросу.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-366">Instead, we use our entities in the application and if additional data is needed Entity Framework will load the data on demand.</span></span>

### <a name="lazy-loading"></a><span data-ttu-id="c3c7b-367">Отложенная загрузка</span><span class="sxs-lookup"><span data-stu-id="c3c7b-367">Lazy Loading</span></span>

<span data-ttu-id="c3c7b-368">Очень просто Представьте себе ситуацию, где мы не знаем, что потребуется данных часть бизнес-логики.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-368">It’s easy to imagine a scenario where we don’t know what data a piece of business logic will need.</span></span> <span data-ttu-id="c3c7b-369">Мы знаем, логика должна объект employee, но мы может выполнять переход в разные пути выполнения которых некоторые из этих путей требуют сведения от сотрудника, а некоторые нет.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-369">We might know the logic needs an employee object, but we may branch into different execution paths where some of those paths require time card information from the employee, and some do not.</span></span> <span data-ttu-id="c3c7b-370">Такие сценарии, как это идеально подходят для неявного отложенной загрузки, так как данные волшебным образом отображаются по мере необходимости.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-370">Scenarios like this are perfect for implicit lazy loading because data magically appears on an as-needed basis.</span></span>

<span data-ttu-id="c3c7b-371">Отложенная загрузка, также известный как отложенная загрузка, поместите некоторые требования объектов сущности.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-371">Lazy loading, also known as deferred loading, does place some requirements on our entity objects.</span></span> <span data-ttu-id="c3c7b-372">Любые требования из уровня сохраняемости не сопряжено с POCO с обеспечением сохраняемости значение true, но true сохраняемости практически невозможно достичь.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-372">POCOs with true persistence ignorance would not face any requirements from the persistence layer, but true persistence ignorance is practically impossible to achieve.</span></span>  <span data-ttu-id="c3c7b-373">Вместо этого мы измеряем сохраняемости в градусах относительный.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-373">Instead we measure persistence ignorance in relative degrees.</span></span> <span data-ttu-id="c3c7b-374">Было бы нежелательным, если нужно наследовать от базового класса ориентированного сохраняемости или использовать специализированные коллекции для достижения отложенной загрузки в POCO.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-374">It would be unfortunate if we needed to inherit from a persistence oriented base class or use a specialized collection to achieve lazy loading in POCOs.</span></span> <span data-ttu-id="c3c7b-375">К счастью EF4 содержит щадящим решения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-375">Fortunately, EF4 has a less intrusive solution.</span></span>

### <a name="virtually-undetectable"></a><span data-ttu-id="c3c7b-376">Практически невозможно обнаружить</span><span class="sxs-lookup"><span data-stu-id="c3c7b-376">Virtually Undetectable</span></span>

<span data-ttu-id="c3c7b-377">При работе с объектами POCO, EF4 может динамически создавать прокси-серверы среды выполнения для сущностей.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-377">When using POCO objects, EF4 can dynamically generate runtime proxies for entities.</span></span> <span data-ttu-id="c3c7b-378">Эти учетные записи-посредники незаметно wrap материализованные POCO и предоставить дополнительные службы, перехватывая каждого свойства, которые получают операций установки и выполнить дополнительные действия.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-378">These proxies invisibly wrap the materialized POCOs and provide additional services by intercepting each property get and set operation to perform additional work.</span></span> <span data-ttu-id="c3c7b-379">Из таких служб — это возможность отложенной загрузки, которую мы ищем.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-379">One such service is the lazy loading feature we are looking for.</span></span> <span data-ttu-id="c3c7b-380">Другая служба — это эффективный механизм, который можно записать, когда она изменяет значения свойств сущности отслеживания изменений.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-380">Another service is an efficient change tracking mechanism which can record when the program changes the property values of an entity.</span></span> <span data-ttu-id="c3c7b-381">Список изменений используется контекст ObjectContext во время выполнения метода SaveChanges для сохранения любого измененных сущностей, с помощью команды обновления.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-381">The list of changes is used by the ObjectContext during the SaveChanges method to persist any modified entities using UPDATE commands.</span></span>

<span data-ttu-id="c3c7b-382">Для этих учетных записей-посредников для работы Однако они нужен способ обработки свойства get и набора операций на сущности и прокси-серверы достижения этой цели путем переопределения виртуальных членов.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-382">For these proxies to work, however, they need a way to hook into property get and set operations on an entity, and the proxies achieve this goal by overriding virtual members.</span></span> <span data-ttu-id="c3c7b-383">Таким образом Если мы хотим использовать неявную отложенную загрузку и эффективное отслеживание изменений необходимо вернуться к нашей определения классов POCO и пометки свойства как виртуальный.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-383">Thus, if we want to have implicit lazy loading and efficient change tracking we need to go back to our POCO class definitions and mark properties as virtual.</span></span>
...
<span data-ttu-id="c3c7b-384">Мы по-прежнему может сообщать, что сущность сотрудника является главным образом игнорирующих сохраняемость.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-384">We can still say the Employee entity is mostly persistence ignorant.</span></span> <span data-ttu-id="c3c7b-385">Единственное требование заключается в том, чтобы использовать виртуальные члены, и это не влияет на пригодность для тестирования кода.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-385">The only requirement is to use virtual members and this does not impact the testability of the code.</span></span> <span data-ttu-id="c3c7b-386">Мы не должны наследовать от любого специального базового класса, или даже использовать специальную подборку, выделенных для отложенной загрузки.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-386">We don’t need to derive from any special base class, or even use a special collection dedicated to lazy loading.</span></span> <span data-ttu-id="c3c7b-387">Как показано в коде, любой класс, реализующий интерфейс ICollection&lt;T&gt; доступен для хранения связанных сущностей.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-387">As the code demonstrates, any class implementing ICollection&lt;T&gt; is available to hold related entities.</span></span>

<span data-ttu-id="c3c7b-388">Имеется также одним небольшим изменением, что необходимо сделать внутри нашей единицей работы.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-388">There is also one minor change we need to make inside our unit of work.</span></span> <span data-ttu-id="c3c7b-389">Отложенная загрузка — это *off* по умолчанию при работе непосредственно с объекта ObjectContext.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-389">Lazy loading is *off* by default when working directly with an ObjectContext object.</span></span> <span data-ttu-id="c3c7b-390">Свойства, которые можно установить для свойства ContextOptions для включения отложенной загрузки, и мы можно установить это свойство внутри наших реальных единица работы, если необходимо, чтобы везде отложенной загрузки.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-390">There is a property we can set on the ContextOptions property to enable deferred loading, and we can set this property inside our real unit of work if we want to enable lazy loading everywhere.</span></span>
...
<span data-ttu-id="c3c7b-391">Неявные отложенная загрузка включена, код приложения можно использовать сотрудника и сотрудника связанных карт время сохраняя при этом не нужно знать о работы, необходимой для Entity FRAMEWORK загрузить дополнительные данные.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-391">With implicit lazy loading enabled, application code can use an employee and the employee’s associated time cards while remaining blissfully unaware of the work required for EF to load the extra data.</span></span>
...
<span data-ttu-id="c3c7b-392">Отложенная загрузка упрощает код приложения для записи и с помощью волшебной команды прокси-сервера остается полностью пригодного для тестирования кода.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-392">Lazy loading makes the application code easier to write, and with the proxy magic the code remains completely testable.</span></span> <span data-ttu-id="c3c7b-393">Fakes в памяти, единицы работы просто можно предварительно загрузить фиктивные сущности с связанные данные, при необходимости во время теста.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-393">In-memory fakes of the unit of work can simply preload fake entities with associated data when needed during a test.</span></span>

<span data-ttu-id="c3c7b-394">На этом этапе мы обратим наше внимание от создания репозиториев с помощью помощью IObjectSet&lt;T&gt; и посмотрите на абстракции, чтобы скрыть все знаки платформы сохраняемости.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-394">At this point we’ll turn our attention from building repositories using IObjectSet&lt;T&gt; and look at abstractions to hide all signs of the persistence framework.</span></span>

## <a name="custom-repositories"></a><span data-ttu-id="c3c7b-395">Пользовательские репозитории</span><span class="sxs-lookup"><span data-stu-id="c3c7b-395">Custom Repositories</span></span>

<span data-ttu-id="c3c7b-396">Когда мы впервые представлен шаблон единицы работы конструктора в этой статье мы предоставили пример кода для как может выглядеть единицу работы.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-396">When we first presented the unit of work design pattern in this article we provided some sample code for what the unit of work might look like.</span></span> <span data-ttu-id="c3c7b-397">Давайте снова представить эту идею исходного, с помощью сотрудников и карта времени ситуацию с сотрудниками, мы работали с.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-397">Let’s re-present this original idea using the employee and employee time card scenario we’ve been working with.</span></span>
...
<span data-ttu-id="c3c7b-398">— Это основное различие между этой единицы работы и единицу работы, мы создали в предыдущем разделе, как эта единица работы не использовать такие абстракции, с помощью EF4 framework (имеется не помощью IObjectSet&lt;T&gt;).</span><span class="sxs-lookup"><span data-stu-id="c3c7b-398">The primary difference between this unit of work and the unit of work we created in the last section is how this unit of work does not use any abstractions from the EF4 framework (there is no IObjectSet&lt;T&gt;).</span></span> <span data-ttu-id="c3c7b-399">Помощью IObjectSet&lt;T&gt; работает, а также интерфейс репозитория, но он предоставляет API могут быть не согласованы полностью с потребностями приложения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-399">IObjectSet&lt;T&gt; works well as a repository interface, but the API it exposes might not perfectly align with our application’s needs.</span></span> <span data-ttu-id="c3c7b-400">При таком подходе предстоящих будем представлять репозитории, с помощью пользовательских IRepository&lt;T&gt; абстракции.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-400">In this upcoming approach we will represent repositories using a custom IRepository&lt;T&gt; abstraction.</span></span>

<span data-ttu-id="c3c7b-401">Многие разработчики, которым следуют разработки через тестирование, проектирование на основе поведения и методологии разработки предметно-ориентированное предпочитают IRepository&lt;T&gt; подход по следующим причинам.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-401">Many developers who follow test-driven design, behavior-driven design, and domain driven methodologies design prefer the IRepository&lt;T&gt; approach for several reasons.</span></span> <span data-ttu-id="c3c7b-402">Во-первых, IRepository&lt;T&gt; интерфейс представляет слой «защиты от повреждения».</span><span class="sxs-lookup"><span data-stu-id="c3c7b-402">First, the IRepository&lt;T&gt; interface represents an “anti-corruption” layer.</span></span> <span data-ttu-id="c3c7b-403">Как описано в своей книге проблемно-ориентированное проектирование (Eric Evans) уровень защиты от повреждения хранит код предметной области от инфраструктуры API-интерфейсы, такие как API сохраняемости.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-403">As described by Eric Evans in his Domain Driven Design book an anti-corruption layer keeps your domain code away from infrastructure APIs, like a persistence API.</span></span> <span data-ttu-id="c3c7b-404">Во-вторых разработчики могут создавать методы в репозиторий, отвечают потребностям приложения, (, обнаруженные во время записи тестов).</span><span class="sxs-lookup"><span data-stu-id="c3c7b-404">Secondly, developers can build methods into the repository that meet the exact needs of an application (as discovered while writing tests).</span></span> <span data-ttu-id="c3c7b-405">Например мы часто может потребоваться найти одну сущность, с помощью значений Идентификаторов, поэтому мы можем добавить метод FindById интерфейс репозитория.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-405">For example, we might frequently need to locate a single entity using an ID value, so we can add a FindById method to the repository interface.</span></span>  <span data-ttu-id="c3c7b-406">Наши IRepository&lt;T&gt; определение будет выглядеть следующим образом.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-406">Our IRepository&lt;T&gt; definition will look like the following.</span></span>
...
<span data-ttu-id="c3c7b-407">Обратите внимание на то, будет отправлено обратно с помощью IQueryable&lt;T&gt; интерфейс для предоставления коллекции сущностей.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-407">Notice we’ll drop back to using an IQueryable&lt;T&gt; interface to expose entity collections.</span></span> <span data-ttu-id="c3c7b-408">IQueryable&lt;T&gt; позволяет деревьях выражений LINQ, поступающие в EF4 поставщика и предоставление поставщика целостное представление запроса.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-408">IQueryable&lt;T&gt; allows LINQ expression trees to flow into the EF4 provider and give the provider a holistic view of the query.</span></span> <span data-ttu-id="c3c7b-409">Второй вариант будет возвращать объект IEnumerable&lt;T&gt;, это означает, поставщик EF4 LINQ будет видеть только выражения, построенные в репозитории.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-409">A second option would be to return IEnumerable&lt;T&gt;, which means the EF4 LINQ provider will only see the expressions built inside of the repository.</span></span> <span data-ttu-id="c3c7b-410">Любой группирования, сортировки и проекции, вне репозитория будет состоять не в команду SQL, отправленную в базу данных, которая может вызвать снижение производительности.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-410">Any grouping, ordering, and projection done outside of the repository will not be composed into the SQL command sent to the database, which can hurt performance.</span></span> <span data-ttu-id="c3c7b-411">С другой стороны, хранилище, возвращая только IEnumerable&lt;T&gt; результаты будут никогда не удивляет с помощью новой команды SQL.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-411">On the other hand, a repository returning only IEnumerable&lt;T&gt; results will never surprise you with a new SQL command.</span></span> <span data-ttu-id="c3c7b-412">Оба подхода будут работать, и оба подхода остаются пригодного для тестирования.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-412">Both approaches will work, and both approaches remain testable.</span></span>

<span data-ttu-id="c3c7b-413">Это просто для предоставления единую реализацию IRepository&lt;T&gt; интерфейс, с помощью универсальных типов и EF4 ObjectContext API.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-413">It’s straightforward to provide a single implementation of the IRepository&lt;T&gt; interface using generics and the EF4 ObjectContext API.</span></span>
...
<span data-ttu-id="c3c7b-414">IRepository&lt;T&gt; подход дает нам некоторые дополнительный контроль над наши запросы так, как клиент должен вызвать метод, чтобы перейти к сущности.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-414">The IRepository&lt;T&gt; approach gives us some additional control over our queries because a client has to invoke a method to get to an entity.</span></span> <span data-ttu-id="c3c7b-415">Внутри метода мы предоставляем дополнительные проверки и операторы LINQ для принудительного применения ограничения для приложения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-415">Inside the method we could provide additional checks and LINQ operators to enforce application constraints.</span></span> <span data-ttu-id="c3c7b-416">Обратите внимание, что интерфейс имеет два ограничения для параметра универсального типа.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-416">Notice the interface has two constraints on the generic type parameter.</span></span> <span data-ttu-id="c3c7b-417">Первое ограничение – недостатки класс нарушить проверку необходимых ObjectSet&lt;T&gt;, и второе ограничение принудительно сущностями для реализации IEntity — это абстракция, созданного для приложения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-417">The first constraint is the class cons taint required by ObjectSet&lt;T&gt;, and the second constraint forces our entities to implement IEntity – an abstraction created for the application.</span></span> <span data-ttu-id="c3c7b-418">Интерфейс IEntity заставляет сущностей в удобном для чтения свойств Id, и можно затем использовать это свойство в метод FindById.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-418">The IEntity interface forces entities to have a readable Id property, and we can then use this property in the FindById method.</span></span> <span data-ttu-id="c3c7b-419">IEntity определяется следующим кодом.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-419">IEntity is defined with the following code.</span></span>
...
<span data-ttu-id="c3c7b-420">IEntity можно считать небольшой нарушение сохраняемости, поскольку сущностями необходимы для реализации этого интерфейса.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-420">IEntity could be considered a small violation of persistence ignorance since our entities are required to implement this interface.</span></span> <span data-ttu-id="c3c7b-421">Помните, принцип независимости сохраняемости очень о необходимости поиска компромиссов, и для многих функций FindById перевешивают ограничение, наложенное интерфейс.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-421">Remember persistence ignorance is about tradeoffs, and for many the FindById functionality will outweigh the constraint imposed by the interface.</span></span> <span data-ttu-id="c3c7b-422">Интерфейс не оказывает влияния на пригодность для тестирования.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-422">The interface has no impact on testability.</span></span>

<span data-ttu-id="c3c7b-423">Создание экземпляра динамической IRepository&lt;T&gt; требуется контекст ObjectContext EF4, поэтому конкретные единицы реализации рабочих должен управлять создание экземпляра.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-423">Instantiating a live IRepository&lt;T&gt; requires an EF4 ObjectContext, so a concrete unit of work implementation should manage the instantiation.</span></span>
...
### <a name="using-the-custom-repository"></a><span data-ttu-id="c3c7b-424">С помощью пользовательского репозитория</span><span class="sxs-lookup"><span data-stu-id="c3c7b-424">Using the Custom Repository</span></span>

<span data-ttu-id="c3c7b-425">С помощью нашего пользовательского репозитория незначительно отличается от использования репозитория, в соответствии с помощью IObjectSet&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-425">Using our custom repository is not significantly different from using the repository based on IObjectSet&lt;T&gt;.</span></span> <span data-ttu-id="c3c7b-426">Вместо применения операторов LINQ непосредственно к свойству, мы сначала необходимо вызвать один методы репозитория, чтобы скопировать IQueryable&lt;T&gt; ссылки.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-426">Instead of applying LINQ operators directly to a property, we’ll first need to invoke one the repository’s methods to grab an IQueryable&lt;T&gt; reference.</span></span>
...
<span data-ttu-id="c3c7b-427">Обратите внимание на то, что пользовательский оператор Include, которую мы применили ранее будет работать без изменений.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-427">Notice the custom Include operator we implemented previously will work without change.</span></span> <span data-ttu-id="c3c7b-428">Метод FindById репозитория удаляет повторяющиеся логики из действия, при попытке извлечь одну сущность.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-428">The repository’s FindById method removes duplicated logic from actions trying to retrieve a single entity.</span></span>
...
<span data-ttu-id="c3c7b-429">Нет нет существенной разницы в двух подходов, которые мы рассмотрели возможности тестирования.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-429">There is no significant difference in the testability of the two approaches we’ve examined.</span></span> <span data-ttu-id="c3c7b-430">Мы можем решить фиктивных реализаций из IRepository&lt;T&gt; , создав конкретные классы, поддерживаемый HashSet&lt;сотрудника&gt; — так же, как мы сделали в последнем разделе.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-430">We could provide fake implementations of IRepository&lt;T&gt; by building concrete classes backed by HashSet&lt;Employee&gt; - just like what we did in the last section.</span></span> <span data-ttu-id="c3c7b-431">Тем не менее некоторые разработчики предпочитают использовать макеты объектов и макетирования платформ объекта вместо сборки fakes.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-431">However, some developers prefer to use mock objects and mock object frameworks instead of building fakes.</span></span> <span data-ttu-id="c3c7b-432">Мы рассмотрим использование макетов для тестирования нашей реализации и обсудим различия между макеты и имитаций в следующем разделе.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-432">We’ll look at using mocks to test our implementation and discuss the differences between mocks and fakes in the next section.</span></span>

### <a name="testing-with-mocks"></a><span data-ttu-id="c3c7b-433">Тестирование с помощью макетов</span><span class="sxs-lookup"><span data-stu-id="c3c7b-433">Testing with Mocks</span></span>

<span data-ttu-id="c3c7b-434">Существуют различные способы создания какие вызовы (Martin Fowler) «тестовый дублер».</span><span class="sxs-lookup"><span data-stu-id="c3c7b-434">There are different approaches to building what Martin Fowler calls a “test double”.</span></span> <span data-ttu-id="c3c7b-435">Тестовый дублер (например, фильма stunt double) — это объект, создаваемых «вместо» для реальных рабочих объектов во время выполнения тестов.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-435">A test double (like a movie stunt double) is an object you build to “stand in” for real, production objects during tests.</span></span> <span data-ttu-id="c3c7b-436">Репозитории в памяти, созданной нами — тестовых дублеров для репозиториев, которые обращаются к SQL Server.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-436">The in-memory repositories we created are test doubles for the repositories that talk to SQL Server.</span></span> <span data-ttu-id="c3c7b-437">Мы узнали, как использовать эти тестовые дублеры во время модульных тестов для изоляции кода и сохранить быстрому запуску тестов.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-437">We’ve seen how to use these test-doubles during the unit tests to isolate code and keep tests running fast.</span></span>

<span data-ttu-id="c3c7b-438">Тестовых дублеров, которые мы создали имеют реализации реальных, работа.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-438">The test doubles we’ve built have real, working implementations.</span></span> <span data-ttu-id="c3c7b-439">За кулисами каждый из них хранит коллекцию объектов, и они будут добавлять и удалять объекты из данной коллекции, как мы управлять хранилище во время теста.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-439">Behind the scenes each one stores a concrete collection of objects, and they will add and remove objects from this collection as we manipulate the repository during a test.</span></span> <span data-ttu-id="c3c7b-440">Некоторые разработчики предпочитаю создавать их тестовых дублеров таким образом — с реальный код и работу реализации.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-440">Some developers like to build their test doubles this way – with real code and working implementations.</span></span>  <span data-ttu-id="c3c7b-441">Эти тестовые дублеры так называемые *fakes*.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-441">These test doubles are what we call *fakes*.</span></span> <span data-ttu-id="c3c7b-442">Они имеют рабочей реализации, но они не реальных, для использования в рабочей среде.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-442">They have working implementations, but they aren’t real enough for production use.</span></span> <span data-ttu-id="c3c7b-443">Имитацией хранилища фактически выполняет запись в базе данных.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-443">The fake repository doesn’t actually write to the database.</span></span> <span data-ttu-id="c3c7b-444">Фиктивные SMTP-сервера не фактически отправляет сообщение электронной почты по сети.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-444">The fake SMTP server doesn’t actually send an email message over the network.</span></span>

### <a name="mocks-versus-fakes"></a><span data-ttu-id="c3c7b-445">Макеты и имитаций</span><span class="sxs-lookup"><span data-stu-id="c3c7b-445">Mocks versus Fakes</span></span>

<span data-ttu-id="c3c7b-446">Есть другой тип теста, double, известный как *макетирование*.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-446">There is another type of test double known as a *mock*.</span></span> <span data-ttu-id="c3c7b-447">Хотя fakes рабочий реализаций, макеты поставляются с нет.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-447">While fakes have working implementations, mocks come with no implementation.</span></span> <span data-ttu-id="c3c7b-448">С помощью платформы макетов объектов создается во время выполнения эти макеты объектов и использовать их в качестве тестовых дублеров.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-448">With the help of a mock object framework we construct these mock objects at run time and use them as test doubles.</span></span> <span data-ttu-id="c3c7b-449">В этом разделе мы будем использовать макетирование framework Moq открытым кодом.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-449">In this section we’ll be using the open source mocking framework Moq.</span></span> <span data-ttu-id="c3c7b-450">Ниже приведен простой пример с помощью Moq для динамического создания теста double для репозиторием сотрудника.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-450">Here is a simple example of using Moq to dynamically create a test double for an employee repository.</span></span>
...
<span data-ttu-id="c3c7b-451">Мы запрашиваем IRepository Moq&lt;сотрудника&gt; реализации и он создает его динамически.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-451">We ask Moq for an IRepository&lt;Employee&gt; implementation and it builds one dynamically.</span></span> <span data-ttu-id="c3c7b-452">Мы можем получить для объекта, реализующего IRepository&lt;сотрудника&gt; путем обращения к свойству объекта фиктивного объекта&lt;T&gt; объекта.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-452">We can get to the object implementing IRepository&lt;Employee&gt; by accessing the Object property of the Mock&lt;T&gt; object.</span></span> <span data-ttu-id="c3c7b-453">Это этот внутренний объект, которые можно передать в наши контроллеры, и он может не знать, если это тестовый дублер или реальные репозитории.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-453">It is this inner object we can pass into our controllers, and they won’t know if this is a test double or the real repository.</span></span> <span data-ttu-id="c3c7b-454">Мы можно вызывать методы в объекте, так же, как мы бы вызывать методы на объект с фактической реализации.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-454">We can invoke methods on the object just like we would invoke methods on an object with a real implementation.</span></span>

<span data-ttu-id="c3c7b-455">Вы должны спросить, что будете выполнять фиктивного репозитория, когда мы вызываем метод Add.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-455">You must be wondering what the mock repository will do when we invoke the Add method.</span></span> <span data-ttu-id="c3c7b-456">Так как не реализацию макета объекта, добавить не выполняет никаких действий.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-456">Since there is no implementation behind the mock object, Add does nothing.</span></span> <span data-ttu-id="c3c7b-457">Нет конкретных коллекции за кулисами, как у нас было с помощью fakes, который был записан, чтобы сотрудник удаляется.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-457">There is no concrete collection behind the scenes like we had with the fakes we wrote, so the employee is discarded.</span></span> <span data-ttu-id="c3c7b-458">Как насчет возвращаемое значение FindById?</span><span class="sxs-lookup"><span data-stu-id="c3c7b-458">What about the return value of FindById?</span></span> <span data-ttu-id="c3c7b-459">В этом случае макета объекта выполняет единственное, что его можно сделать, это возвращаемое значение по умолчанию.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-459">In this case the mock object does the only thing it can do, which is return a default value.</span></span> <span data-ttu-id="c3c7b-460">Так как мы возвращается ссылочным типом (сотрудник), возвращается значение null.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-460">Since we are returning a reference type (an Employee), the return value is a null value.</span></span>

<span data-ttu-id="c3c7b-461">Макеты может показаться бесполезным; Тем не менее существуют две дополнительные возможности макетов, которые мы не затрагивали.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-461">Mocks might sound worthless; however, there are two more features of mocks we haven’t talked about.</span></span> <span data-ttu-id="c3c7b-462">Во-первых Moq framework записывает все вызовы, выполняемые для макета объекта.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-462">First, the Moq framework records all the calls made on the mock object.</span></span> <span data-ttu-id="c3c7b-463">Далее в коде мы просим Moq Если любой пользователь вызывается метод Add, или в том случае, если любой пользователь вызывает метод FindById.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-463">Later in the code we can ask Moq if anyone invoked the Add method, or if anyone invoked the FindById method.</span></span> <span data-ttu-id="c3c7b-464">Мы рассмотрим, как можно использовать эту функцию «черного ящика» запись в тестах.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-464">We’ll see later how we can use this “black box” recording feature in tests.</span></span>

<span data-ttu-id="c3c7b-465">Второй отличная возможность заключается в том, как можно использовать Moq программировать макет объекта с *ожидания*.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-465">The second great feature is how we can use Moq to program a mock object with *expectations*.</span></span> <span data-ttu-id="c3c7b-466">Ожидание указывает макета объекта как отвечать на любые данного действия.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-466">An expectation tells the mock object how to respond to any given interaction.</span></span> <span data-ttu-id="c3c7b-467">Например мы программировать ожидание в наш макет и сообщить его, чтобы вернуть объект employee, когда кто-то вызывает FindById.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-467">For example, we can program an expectation into our mock and tell it to return an employee object when someone invokes FindById.</span></span> <span data-ttu-id="c3c7b-468">Moq инфраструктура использует API установки и лямбда-выражения программировать эти ожидания.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-468">The Moq framework uses a Setup API and lambda expressions to program these expectations.</span></span>
...
<span data-ttu-id="c3c7b-469">В этом примере мы просим Moq динамически создать репозиторий, а затем мы используем при программировании репозиторий с Ожидание.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-469">In this sample we ask Moq to dynamically build a repository, and then we program the repository with an expectation.</span></span> <span data-ttu-id="c3c7b-470">Исходя из предположения о том, макета объекта для возврата объекта сотрудника с идентификатором 5, когда кто-то вызывает метод FindById, передавая значение 5.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-470">The expectation tells the mock object to return a new employee object with an Id value of 5 when someone invokes the FindById method passing a value of 5.</span></span> <span data-ttu-id="c3c7b-471">Этот тест проходит успешно, и нам не нужно создавать полную реализацию, чтобы поддельное IRepository&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-471">This test passes, and we didn’t need to build a full implementation to fake IRepository&lt;T&gt;.</span></span>

<span data-ttu-id="c3c7b-472">Давайте вернемся к тестов, который был записан ранее и переработать их на использование макетов вместо fakes.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-472">Let’s revisit the tests we wrote earlier and rework them to use mocks instead of fakes.</span></span> <span data-ttu-id="c3c7b-473">Так же, как раньше, мы будем использовать базовый класс для настройки общие части инфраструктуры, которые нужны для всех тестов контроллера.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-473">Just like before, we’ll use a base class to setup the common pieces of infrastructure we need for all of the controller’s tests.</span></span>
...
<span data-ttu-id="c3c7b-474">Программный код установки большей части остается неизменным.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-474">The setup code remains mostly the same.</span></span> <span data-ttu-id="c3c7b-475">Вместо использования fakes, мы будем использовать Moq для создания макетов объектов.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-475">Instead of using fakes, we’ll use Moq to construct mock objects.</span></span> <span data-ttu-id="c3c7b-476">Базовый класс упорядочивает макетов единицу работы для возврата фиктивного репозитория, когда код вызывает свойство сотрудников.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-476">The base class arranges for the mock unit of work to return a mock repository when code invokes the Employees property.</span></span> <span data-ttu-id="c3c7b-477">Остальная часть макета настройки будет иметь место внутри средства тестирования, выделенных для каждого конкретного сценария.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-477">The rest of the mock setup will take place inside the test fixtures dedicated to each specific scenario.</span></span> <span data-ttu-id="c3c7b-478">Например средство тестирования для действия индекса настроим фиктивного репозитория, возвращающий список сотрудников, когда действие вызывает метод FindAll фиктивного репозитория.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-478">For example, the test fixture for the Index action will setup the mock repository to return a list of employees when the action invokes the FindAll method of the mock repository.</span></span>
...
<span data-ttu-id="c3c7b-479">За исключением ожиданий наши тесты выглядеть тесты, которые мы использовали ранее.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-479">Except for the expectations, our tests look similar to the tests we had before.</span></span> <span data-ttu-id="c3c7b-480">Тем не менее с возможностью записи макетов платформы мы может приблизить тестирования с другого ракурса.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-480">However, with the recording ability of a mock framework we can approach testing from a different angle.</span></span> <span data-ttu-id="c3c7b-481">Мы рассмотрим это новая Перспектива в следующем разделе.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-481">We’ll look at this new perspective in the next section.</span></span>

### <a name="state-versus-interaction-testing"></a><span data-ttu-id="c3c7b-482">Состояния и тестирование взаимодействия</span><span class="sxs-lookup"><span data-stu-id="c3c7b-482">State versus Interaction Testing</span></span>

<span data-ttu-id="c3c7b-483">Существуют различные методы, которые можно использовать для тестирования программного обеспечения макеты объектов.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-483">There are different techniques you can use to test software with mock objects.</span></span> <span data-ttu-id="c3c7b-484">Одним из подходов является использование состояний на основе тестирования, это, что мы проделали в настоящей документации в данный момент.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-484">One approach is to use state based testing, which is what we have done in this paper so far.</span></span> <span data-ttu-id="c3c7b-485">Состояние на основе тестирования делает утверждения о состоянии программного обеспечения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-485">State based testing makes assertions about the state of the software.</span></span> <span data-ttu-id="c3c7b-486">В последней проверки мы вызова метода действия контроллера и утверждение о модели, он должен быть построен.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-486">In the last test we invoked an action method on the controller and made an assertion about the model it should build.</span></span> <span data-ttu-id="c3c7b-487">Ниже приведены некоторые другие примеры тестирования состояния.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-487">Here are some other examples of testing state:</span></span>

-   <span data-ttu-id="c3c7b-488">Убедитесь, что хранилище содержит новый объект employee, после выполнения Create.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-488">Verify the repository contains the new employee object after Create executes.</span></span>
-   <span data-ttu-id="c3c7b-489">Убедитесь, что модель содержит список всех сотрудников, после выполнения индекса.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-489">Verify the model holds a list of all employees after Index executes.</span></span>
-   <span data-ttu-id="c3c7b-490">Убедитесь, что хранилище не содержит данного сотрудника после выполнения удаления.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-490">Verify the repository does not contain a given employee after Delete executes.</span></span>

<span data-ttu-id="c3c7b-491">Другой подход, вы увидите с макеты объектов — для проверки *взаимодействия*.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-491">Another approach you’ll see with mock objects is to verify *interactions*.</span></span> <span data-ttu-id="c3c7b-492">Хотя состояния на основе тестирования делает предположения о состоянии объектов, взаимодействие на основе тестирования делает утверждения о взаимодействии объектов.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-492">While state based testing makes assertions about the state of objects, interaction based testing makes assertions about how objects interact.</span></span> <span data-ttu-id="c3c7b-493">Пример:</span><span class="sxs-lookup"><span data-stu-id="c3c7b-493">For example:</span></span>

-   <span data-ttu-id="c3c7b-494">Убедитесь, что контроллер вызывает метод Add репозитория при выполнении Create.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-494">Verify the controller invokes the repository’s Add method when Create executes.</span></span>
-   <span data-ttu-id="c3c7b-495">Убедитесь, что контроллер вызывает метод FindAll репозитория, при выполнении индекса.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-495">Verify the controller invokes the repository’s FindAll method when Index executes.</span></span>
-   <span data-ttu-id="c3c7b-496">Убедитесь, что контроллер вызывает единицы работы метода Commit для сохранения изменений, при выполнении редактирования.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-496">Verify the controller invokes the unit of work’s Commit method to save changes when Edit executes.</span></span>

<span data-ttu-id="c3c7b-497">Тестирование взаимодействия часто требуется меньше проверочных данных, так как мы не поковыряться внутри коллекции, а также проверка счетчиков.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-497">Interaction testing often requires less test data, because we aren’t poking inside of collections and verifying counts.</span></span> <span data-ttu-id="c3c7b-498">Например если мы знаем, что на действие Details вызывает метод FindById репозитория с правильным значением - нажмите действие, вероятно, работает правильно.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-498">For example, if we know the Details action invokes a repository’s FindById method with the correct value - then the action is probably behaving correctly.</span></span> <span data-ttu-id="c3c7b-499">Мы можно проверить это без настройки любые тестовые данные для возврата из FindById.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-499">We can verify this behavior without setting up any test data to return from FindById.</span></span>
...
<span data-ttu-id="c3c7b-500">Только установки, необходимый в выше средство тестирования состоит в настройке, предоставляемых базовым классом.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-500">The only setup required in the above test fixture is the setup provided by the base class.</span></span> <span data-ttu-id="c3c7b-501">Когда вызывается действие контроллера, Moq приводит к записи взаимодействия с фиктивного репозитория.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-501">When we invoke the controller action, Moq will record the interactions with the mock repository.</span></span> <span data-ttu-id="c3c7b-502">С помощью API проверки из Moq, мы можете попросить Moq Если контроллера вызываются FindById с правильным значением идентификатора.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-502">Using the Verify API of Moq, we can ask Moq if the controller invoked FindById with the proper ID value.</span></span> <span data-ttu-id="c3c7b-503">Если контроллер не происходит вызов метода, или вызывается метод со значением непредвиденный параметр, проверьте метод вызовет исключение, и тест завершится ошибкой.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-503">If the controller did not invoke the method, or invoked the method with an unexpected parameter value, the Verify method will throw an exception and the test will fail.</span></span>

<span data-ttu-id="c3c7b-504">Вот еще один пример, чтобы убедиться, что действие создания вызывает фиксации в текущей единице работы.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-504">Here is another example to verify the Create action invokes Commit on the current unit of work.</span></span>
...
<span data-ttu-id="c3c7b-505">Один недостаток тестирование взаимодействия является тенденции к через определяют взаимодействие.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-505">One danger with interaction testing is the tendency to over specify interactions.</span></span> <span data-ttu-id="c3c7b-506">Способность макета объекта для записи и убедитесь, что любое взаимодействие с макета объекта не значит, тест следует попытаться проверить каждого диалога.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-506">The ability of the mock object to record and verify every interaction with the mock object doesn’t mean the test should try to verify every interaction.</span></span> <span data-ttu-id="c3c7b-507">Некоторые взаимодействия являются подробностями реализации и необходимо только проверить взаимодействия *требуется* для удовлетворения текущего теста.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-507">Some interactions are implementation details and you should only verify the interactions *required* to satisfy the current test.</span></span>

<span data-ttu-id="c3c7b-508">Выбор между макеты и имитаций во многом зависит от системы, тестировании и личную (или объединении адаптеров) предпочтения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-508">The choice between mocks or fakes largely depends on the system you are testing and your personal (or team) preferences.</span></span> <span data-ttu-id="c3c7b-509">Макеты объектов может значительно сократить объем кода, необходимо реализовать тестовых дублеров, но не всем удобна, программирование ожиданиям и проверке взаимодействия.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-509">Mock objects can drastically reduce the amount of code you need to implement test doubles, but not everyone is comfortable programming expectations and verifying interactions.</span></span>

## <a name="conclusions"></a><span data-ttu-id="c3c7b-510">Выводы</span><span class="sxs-lookup"><span data-stu-id="c3c7b-510">Conclusions</span></span>

<span data-ttu-id="c3c7b-511">В этом документе мы рассмотрели несколько подходов к созданию кода для тестирования при использовании ADO.NET Entity Framework для постоянного хранения.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-511">In this paper we’ve demonstrated several approaches to creating testable code while using the ADO.NET Entity Framework for data persistence.</span></span> <span data-ttu-id="c3c7b-512">Можно использовать встроенные абстракций наподобие помощью IObjectSet&lt;T&gt;, или создания собственных абстракций наподобие IRepository&lt;T&gt;.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-512">We can leverage built in abstractions like IObjectSet&lt;T&gt;, or create our own abstractions like IRepository&lt;T&gt;.</span></span>  <span data-ttu-id="c3c7b-513">В обоих случаях поддержка POCO в Entity Framework ADO.NET 4.0 позволяет пользователям эти абстракции сохраняются неведении и широкими возможностями тестирования.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-513">In both cases, the POCO support in the ADO.NET Entity Framework 4.0 allows the consumers of these abstractions to remain persistent ignorant and highly testable.</span></span> <span data-ttu-id="c3c7b-514">Дополнительные возможности EF4 неявную отложенную загрузку позволяет приложению и бизнес-службы, как код работать, не заботясь о деталях реляционного хранилища данных.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-514">Additional EF4 features like implicit lazy loading allows business and application service code to work without worrying about the details of a relational data store.</span></span> <span data-ttu-id="c3c7b-515">Наконец эти абстракции, которые будут созданы просты в макетируйте и не Имитируйте внутри модульных тестов, и мы можем использовать эти тестовые дублеры для достижения быстрое выполнение, высокой изолированным и надежные тесты.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-515">Finally, the abstractions we create are easy to mock or fake inside of unit tests, and we can use these test doubles to achieve fast running, highly isolated, and reliable tests.</span></span>

### <a name="additional-resources"></a><span data-ttu-id="c3c7b-516">Дополнительные ресурсы</span><span class="sxs-lookup"><span data-stu-id="c3c7b-516">Additional Resources</span></span>

-   <span data-ttu-id="c3c7b-517">Роберт с. Мартин, " [принцип единственной обязанности](http://www.objectmentor.com/resources/articles/srp.pdf)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-517">Robert C. Martin, “ [The Single Responsibility Principle](http://www.objectmentor.com/resources/articles/srp.pdf)”</span></span>
-   <span data-ttu-id="c3c7b-518">(Martin Fowler) [каталог шаблонов](http://www.martinfowler.com/eaaCatalog/index.html) из *шаблоны Enterprise Application Architecture*</span><span class="sxs-lookup"><span data-stu-id="c3c7b-518">Martin Fowler, [Catalog of Patterns](http://www.martinfowler.com/eaaCatalog/index.html) from *Patterns of Enterprise Application Architecture*</span></span>
-   <span data-ttu-id="c3c7b-519">Caprio Гриффин " [внедрения зависимостей](https://msdn.microsoft.com/magazine/cc163739.aspx)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-519">Griffin Caprio, “ [Dependency Injection](https://msdn.microsoft.com/magazine/cc163739.aspx)”</span></span>
-   <span data-ttu-id="c3c7b-520">Блог программистов данных, « [Пошаговое руководство: разработка с помощью Entity Framework 4.0 на основе тестирования](http://blogs.msdn.com/adonet/pages/walkthrough-test-driven-development-with-the-entity-framework-4-0.aspx)«.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-520">Data Programmability Blog, “ [Walkthrough: Test Driven Development with the Entity Framework 4.0](http://blogs.msdn.com/adonet/pages/walkthrough-test-driven-development-with-the-entity-framework-4-0.aspx)”.</span></span>
-   <span data-ttu-id="c3c7b-521">Блог программистов данных, « [шаблонов с помощью репозитория и единица работы с Entity Framework 4.0](http://blogs.msdn.com/adonet/archive/2009/06/16/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0.aspx)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-521">Data Programmability Blog, “ [Using Repository and Unit of Work patterns with Entity Framework 4.0](http://blogs.msdn.com/adonet/archive/2009/06/16/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0.aspx)”</span></span>
-   <span data-ttu-id="c3c7b-522">Дейв Astels, " [введение BDD](http://blog.daveastels.com/files/BDD_Intro.pdf)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-522">Dave Astels, “ [BDD Intro](http://blog.daveastels.com/files/BDD_Intro.pdf)”</span></span>
-   <span data-ttu-id="c3c7b-523">Йенсен Аарон " [Общие сведения о спецификации машин](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-523">Aaron Jensen, “ [Introducing Machine Specifications](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)”</span></span>
-   <span data-ttu-id="c3c7b-524">Эрик ли " [BDD с использованием MSTest](http://blogs.msdn.com/elee/archive/2009/01/20/bdd-with-mstest.aspx)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-524">Eric Lee, “ [BDD with MSTest](http://blogs.msdn.com/elee/archive/2009/01/20/bdd-with-mstest.aspx)”</span></span>
-   <span data-ttu-id="c3c7b-525">(Eric Evans), " [предметно-ориентированное проектирование](http://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-525">Eric Evans, “ [Domain Driven Design](http://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)”</span></span>
-   <span data-ttu-id="c3c7b-526">(Martin Fowler) « [макетов не только заглушки](http://martinfowler.com/articles/mocksArentStubs.html)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-526">Martin Fowler, “ [Mocks Aren’t Stubs](http://martinfowler.com/articles/mocksArentStubs.html)”</span></span>
-   <span data-ttu-id="c3c7b-527">(Martin Fowler) « [тестирования двойной](http://martinfowler.com/bliki/TestDouble.html)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-527">Martin Fowler, “ [Test Double](http://martinfowler.com/bliki/TestDouble.html)”</span></span>
-   <span data-ttu-id="c3c7b-528">Джереми Миллер " [состояние и тестирование взаимодействия](http://codebetter.com/blogs/jeremy.miller/articles/129544.aspx)"</span><span class="sxs-lookup"><span data-stu-id="c3c7b-528">Jeremy Miller, “ [State versus Interaction Testing](http://codebetter.com/blogs/jeremy.miller/articles/129544.aspx)”</span></span>
-   <span data-ttu-id="c3c7b-529">MOQ, \<http://code.google.com/p/moq/></span><span class="sxs-lookup"><span data-stu-id="c3c7b-529">Moq, \<http://code.google.com/p/moq/></span></span>

### <a name="biography"></a><span data-ttu-id="c3c7b-530">Биография</span><span class="sxs-lookup"><span data-stu-id="c3c7b-530">Biography</span></span>

<span data-ttu-id="c3c7b-531">Скотт Аллен является членом технического персонала в Pluralsight и учредитель OdeToCode.com.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-531">Scott Allen is a member of the technical staff at Pluralsight and the founder of OdeToCode.com.</span></span> <span data-ttu-id="c3c7b-532">В 15-летним коммерческой разработки программных продуктов Скотт участвовал в решения для всех операций из 8-разрядных устройств embedded позволяет высокомасштабируемых веб-приложений ASP.NET.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-532">In 15 years of commercial software development, Scott has worked on solutions for everything from 8-bit embedded devices to highly scalable ASP.NET web applications.</span></span> <span data-ttu-id="c3c7b-533">Скоттом можно связаться на его блог по адресу OdeToCode или в Твиттере \< http://twitter.com/OdeToCode>.</span><span class="sxs-lookup"><span data-stu-id="c3c7b-533">You can reach Scott on his blog at OdeToCode, or on Twitter at \<http://twitter.com/OdeToCode>.</span></span>
