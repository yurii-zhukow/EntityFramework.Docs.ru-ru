...
Перед выполнением этого пошагового руководства убедитесь, что вы читаете страницу [сущностей с самостоятельным отслеживанием](index.md) .
...
-   Выберите **\#шаблон тавить для шаблона C** , введите **стетемплате** в качестве имени и нажмите кнопку **добавить** .
...
>[!NOTE]
> Другим вариантом перемещения типов сущностей в отдельный проект является перемещение файла шаблона вместо его связи с расположением по умолчанию. В этом случае необходимо обновить переменную **inputFile** в шаблоне, чтобы указать относительный путь к EDMX-файлу (в данном примере это **..\\блоггингмодел. EDMX**).
...
-   **Файл-&gt; проект Add-&gt;...**
-   Выберите **Visual C\#** на левой панели, а затем — **приложение службы WCF** .
...
-   Вставьте элемент **connectionStrings** в качестве дочернего элемента элемента **конфигурации** файла **Web. config** в проекте **стесампле. Service.**
...
-   **Файл-&gt; проект New-&gt;...**
-   Выберите **Visual C\#** на левой панели, а затем — **приложение WPF**
