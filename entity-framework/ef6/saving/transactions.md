...
Начиная с EF6 **Database. ексекутесклкомманд ()** по умолчанию будет заключить команду в транзакцию, если она еще не была найдена. Существует перегрузки этого метода, позволяющие переопределить это поведение при необходимости. Кроме того, в EF6 выполнение хранимых процедур, входящих в модель с помощью API-интерфейсов, таких как **ObjectContext. ExecuteFunction ()** , выполняет те же действия (за исключением того, что поведение по умолчанию не может быть переопределено).  
