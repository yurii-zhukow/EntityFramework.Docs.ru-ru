...
- Запрос: Регрессия в EF 6.1.2: ВНЕШНее применение появилось и более сложных запросов для связей 1:1 и предложения let
...
- Дизайнеров Ошибка при открытии EF5 EDMX с десятичной точностью в конструкторе EF6
...
- **[Индексаттрибуте](~/ef6/modeling/code-first/data-annotations.md)** позволяет указать индексы, размещая `[Index]` атрибут в свойстве (или свойствах) в модели Code First. Code First создаст соответствующий индекс в базе данных.
...
- Подключаемая служба несамостоятельных несамостоятельных модулей. **&amp;**
...
- С помощью **[пользовательских операций миграции](http://romiller.com/2013/02/27/ef6-writing-your-own-code-first-migration-operations/)** можно добавить дополнительные операции, которые будут использоваться при переносе на основе кода.
...
Ниже приведен список содержимого, которое мы собрали для выпуска EF 4,2. содержимое, предоставленное для EF 4,1, по-прежнему применимо также к EF 4,2.
