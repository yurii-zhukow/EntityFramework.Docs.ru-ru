...
 
## <a name="association-element-csdl"></a>Элемент Association (язык CSDL)
...
| Имя атрибута  | Обязательный | Значение                                                                                                                                                             |
|:----------------|:------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Название**        | Да         | Имя набора сущностей. Значение атрибута **Name** не может совпадать со значением атрибута **Association** .                                 |
...
В следующем примере показан элемент **Documentation** в качестве дочернего элемента элемента EntityType. Если приведенный ниже фрагмент находится в CSDL-файле EDMX-файла, содержимое элементов **Summary** и **лонгдескриптион** будет отображаться в окне **свойств** Visual Studio при щелчке `Customer` типа сущности.
...
## <a name="entitycontainer-element-csdl"></a>Элемент EntityContainer (язык CSDL)
...
| Имя атрибута | Обязательный | Значение                                                           |
|:---------------|:------------|:----------------------------------------------------------------|
| **Название**       | Да         | Имя контейнера сущностей.                               |
...
## <a name="entitytype-element-csdl"></a>Элемент EntityType (язык CSDL)
...
| Имя атрибута   | Обязательный | Значение                                                                                                                                                                                                 |
|:-----------------|:------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Название**         | Да         | Имя импортируемой функции.                                                                                                                                                                    |
...
| Имя атрибута | Обязательный | Значение                                                                                                                                                                                    |
|:---------------|:------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Название**       | Да         | Имя элемента.                                                                                                                                                                  |
| **Value**      | нет          | Значение элемента. По умолчанию первый элемент имеет значение 0, а значение каждого последующего перечислителя увеличивается на 1. Могут существовать несколько членов с одинаковыми значениями. |
...
## <a name="navigationproperty-element-csdl"></a>Элемент NavigationProperty (язык CSDL)
...
| Имя атрибута   | Обязательный | Значение                                                                                                                                                                                                                                            |
|:-----------------|:------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Название**         | Да         | Имя свойства навигации.                                                                                                                                                                                                             |
...
Элемент **ReferentialConstraint** в языке CSDL определяет функциональные возможности, аналогичные ограничению ссылочной целостности в реляционной базе данных. Так же как столбец (или столбцы) из таблицы базы данных может иметь ссылку на первичный ключ другой таблицы, свойство (или свойства) типа сущности может иметь ссылки на ключ сущности другого типа сущности. Тип сущности, на который указывает ссылка, называется *основным элементом* ограничения. Тип сущности, который ссылается на основной элемент, называется *зависимым окончанием* ограничения.
...
| Имя атрибута | Обязательный | Значение                                                                                                                                                                              |
|:---------------|:------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Пространство имен**  | Да         | Имя импортированного пространства имен.                                                                                                                                                |
...
В следующем примере показан элемент **using** , используемый для импорта пространства имен, определенного в других местах. Обратите внимание, что пространство имен для отображаемого элемента **схемы** `BooksModel`. Свойство `Address` на `Publisher`**EntityType** — сложный тип, определенный в пространстве имен `ExtendedBooksModel` (импортированном с **помощью** элемента).
...
| **Edm.DateTimeOffset**           | Содержит дату и время в виде смещения в минутах от времени GMT. | Precision, Nullable, Default                                             |
