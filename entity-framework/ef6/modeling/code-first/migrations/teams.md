...
---
# <a name="code-first-migrations-in-team-environments"></a>Code First Migrations в средах групп
> [!NOTE]
...
При миграции используется таблица **\_ @ no__t-2MigrationsHistory** для хранения данных, к которым применены миграции. Если у вас есть несколько разработчиков, создающих различные миграции при попытке ориентироваться на одну и ту же базу данных (и, таким является, совместное использование **\_ @ no__t-2MigrationsHistory** Table), миграция будет очень запутанной.
...
### <a name="video-one-migrations---under-the-hood"></a>Один видеоролик: «Миграция — внутри себя»
...
### <a name="video-two-migrations---team-environments"></a>Видео два: "Миграции — среды группы"
...
Разработчик \#1 и разработчик \#2 теперь вносит некоторые изменения в модель EF в своей локальной базе кода. Разработчик \#1 добавляет свойство **рейтинга** в **блог** — и создает миграцию **аддратинг** , чтобы применить изменения к базе данных. Разработчик \#2 добавляет в **блог** свойство " **читатели** " и создает соответствующую миграцию **аддреадерс** . Оба разработчика выполняют **Обновление базы данных**, применяют изменения к их локальным базам данных, а затем продолжают разработку приложения.

> [!NOTE]
> Для миграции используется префикс отметок времени, поэтому наш график означает, что Аддреадерс миграция от разработчика \#2 появилась после Аддратинг миграции от Developer \#1. Как разработчик \#1, так и \#2, впервые они не отличаются от проблем, связанных с работой в команде, или процесса слияния, который мы рассмотрим в следующем разделе.

![Локальные изменения](~/ef6/media/localchanges.png)

Это счастливого дня для разработчиков \#1, так как они поправляют свои изменения первыми. Так как никто из других пользователей не вернул, так как он синхронизирует свой репозиторий, он может просто отправить свои изменения без выполнения слияния.

![Отправить](~/ef6/media/submit.png)

Пришло время для отправки разработчику \#2. Они не так счастливы. Так как кто-то другой отправил изменения с момента их синхронизации, им потребуется извлечь изменения и выполнить слияние. Система управления версиями, скорее всего, сможет автоматически объединить изменения на уровне кода, так как они очень просты. Состояние локального репозитория Developer \#2 после синхронизации показано на следующем рисунке. 

![Собирает](~/ef6/media/pull.png)

На этом этапе разработчик \#2 может выполнить **Update-Database** , который определит новую миграцию **аддратинг** (которая не была применена к базе данных Developer \#2) и применит ее. Теперь столбец **рейтинга** добавляется в таблицу **блоги** , и база данных синхронизируется с моделью.

Существует несколько проблем:

1.  Хотя **Обновление базы данных** будет применять **аддратинг** миграцию, оно также выдаст предупреждение: *Не удалось обновить базу данных в соответствии с текущей моделью, так как имеются ожидающие изменения и отключена автоматическая миграция...*
...
2.  Запуск приложения приведет к тому, что "*The модель резервного копирования контекста" BloggingContext "изменилась с момента создания базы данных. Попробуйте использовать Code First Migrations для обновления базы данных... "*
...
### <a name="option-1-add-a-blank-merge-migration"></a>Вариант 1: Добавить пустую миграцию "слиянием"
...
Этот параметр можно использовать независимо от того, кто создал последнюю миграцию. В примере мы уже работали с разработчиком \#2 при слиянии, и это произошло для создания последней миграции. Но эти же действия можно использовать, если разработчик \#1 создал последнюю миграцию. Эти шаги также применяются, если участвуют несколько миграций — мы только что видели два, чтобы они были простыми.
...
4.  Выполните команду **Add-Migration &lt;pick @ no__t-2A @ no__t-3name @ no__t-4-игноречанжес** (например, **Слияние с добавлением миграции — игноречанжес**). При этом создается миграция со всеми метаданными (включая моментальный снимок текущей модели), но игнорируются все изменения, обнаруженные при сравнении текущей модели с моментальным снимком при последнем переносе (это означает, что вы получаете пустой метод " **вверх** и **вниз** ").
5.  Продолжить разработку или отправить в систему управления версиями (после выполнения модульных тестов курса).

Ниже приведено состояние Developer \#2 в локальной базе кода после использования этого подхода.
...
3.  Выполните **Обновление базы данных** , чтобы применить новые миграции, которые были возвращены другими разработчиками.
    **_Примечание._** *Если вы не получаете предупреждения от команды Update-Database, то не существовали новые миграции от других разработчиков и нет необходимости выполнять дальнейшее слияние.*
4.  Выполните **Update-Database — таржетмигратион &lt;second @ no__t-2last @ no__t-3migration @ no__t-4** (в примере мы уже следовали бы **Обновить-Database – таржетмигратион аддратинг**). Эта роль применяет базу данных к состоянию второй последней миграции — фактически "Отмена применения" последней миграции из базы данных.
    **_Метим_** @no__t 0This требуется, чтобы обеспечить безопасность изменения метаданных миграции, так как метаданные также хранятся в \_ @ no__t-2MigrationsHistoryTable базы данных. Именно поэтому этот параметр следует использовать только в том случае, если последняя миграция выполняется только в локальной базе кода. Если в других базах данных была применена последняя миграция, также потребуется выполнить откат и повторно применить последнюю миграцию для обновления метаданных. * 
5.  Выполните команду **Add-migration &lt;full @ no__t-2name @ no__t-3including @ no__t-4timestamp @ no__t-5of @ no__t-6last @ no__t-** 7migration &gt; (в приведенном выше примере это будет нечто вроде **Add-Migration 201311062215252 @ no__ t-10AddReaders**).
    **_Метим_** *Необходимо включить метку времени, чтобы при миграции было известно, что необходимо изменить существующую миграцию, а не создать формирование шаблонов.*
    При этом метаданные для последней миграции будут обновлены в соответствии с текущей моделью. При завершении команды вы получите следующее предупреждение, но это именно то, что вам нужно. "*Only код конструктора для миграции" 201311062215252 @ no__t-1AddReaders "был изменен с помощью шаблона. Чтобы изменить формирование шаблонов для всей миграции, используйте параметр-Force.*
