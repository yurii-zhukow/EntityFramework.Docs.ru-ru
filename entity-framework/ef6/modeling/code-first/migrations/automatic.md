...
Мы будем избегать использования добавления-миграции (если это не требуется) и сосредоточиться на том, чтобы Code First Migrations автоматически вычислять и применять изменения. Мы используем **Обновление базы данных** , чтобы получить Code First migrations для отправки изменений в нашу модель (новое свойство **Blog. ur**l) в базу данных.
...
Воспользуемся командой Add-Migration, чтобы записать это изменение в перенос на основе кода, чтобы мы могли изменить его. С помощью команды **Add-Migration** можно присвоить имя миграции. Давайте просто вызываем наш **аддблогратинг**.
