...
### <a name="video-two-migrations---existing-databases"></a>Видео два: «Миграция — существующие базы данных»
...
## <a name="step-2-enable-migrations"></a>Шаг 2. Включить миграции
...
2.  Выполните команду **обновления базы данных** в консоли диспетчера пакетов. Это приведет к применению InitialCreate миграции к базе данных. Поскольку фактическая миграция не содержит каких-либо изменений, она просто добавляет строку в таблицу \_ @ no__t-1MigrationsHistory, указывающую на то, что эта миграция уже применена.

### <a name="option-two-use-empty-database-as-a-starting-point"></a>Вариант 2: Использовать пустую базу данных в качестве отправной точки
...
3.  Выполните команду **обновления базы данных** в консоли диспетчера пакетов. Это приведет к применению InitialCreate миграции к базе данных. Поскольку фактическая миграция не содержит каких-либо изменений (так как мы временно закомментируем их), она просто добавляет строку в таблицу \_ @ no__t-1MigrationsHistory, указывающую на то, что эта миграция уже применена.
...
## <a name="things-to-be-aware-of"></a>Вопросы, которые следует учитывать
...
**If вы использовали "вариант 1: Использовать существующую схему в качестве отправной точки "из шага 3:**
...
-   Если будущие изменения в модели требуют изменения или удаления одного из объектов базы данных, названных по-другому, необходимо изменить шаблонную миграцию, указав правильное имя. API-интерфейсы миграции имеют необязательный параметр name, позволяющий сделать это.
    Например, существующая схема может содержать таблицу Post со столбцом внешнего ключа Блогид, имеющего индекс с именем Индексфк @ no__t-0BlogId. Однако при миграции по умолчанию предполагается, что этот индекс будет называться IX @ no__t-0BlogId. При внесении изменений в модель, которая приводит к удалению этого индекса, необходимо изменить событие DropIndex вызов с формированием шаблонов, указав имя Индексфк @ no__t-0BlogId.

**If, использованный в двух вариантах: Использовать пустую базу данных в качестве отправной точки "из шага 3:**
...
-   Если вы использовали параметр "два: Использовать пустую базу данных в качестве отправной точки. Эти дополнительные объекты не будут созданы методом первоначальной миграции.
