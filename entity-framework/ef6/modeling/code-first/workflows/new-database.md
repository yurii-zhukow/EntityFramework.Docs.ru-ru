...
Теперь пора определить производный контекст, который представляет сеанс с базой данных, что позволит нам запрашивать и сохранять данные. Мы определяем контекст, производный от System. Data. Entity. DbContext, и предоставляем типизированный DbSet&lt;&gt; для каждого класса в нашей модели.
...
-   **Сервис-&gt; диспетчер пакетов библиотек — консоль диспетчера пакетов&gt;**
...
Чтобы получить доступ к API Fluent, переопределите метод OnModelCreating в DbContext. Предположим, что нам нужно переименовать столбец, в котором хранится User. DisplayName, чтобы отображалось имя\_.
...
Столбец DisplayName теперь переименован для вывода имени\_:
