...
При использовании Visual Studio рассмотрите возможность использования [средств консоли диспетчера пакетов](xref:core/cli/powershell) вместо средств CLI. Средства консоли диспетчера пакетов автоматически:
...
Чтобы использовать его в качестве локального средства, восстановите зависимости проекта, в котором оно объявляется в качестве соответствующей зависимости, с помощью [файла манифеста средства](/dotnet/core/tools/global-tools#install-a-local-tool).
...
Чтобы указать [среду](/aspnet/core/fundamentals/environments) для проектов ASP.NET Core, задайте переменную среды **ASPNETCORE_ENVIRONMENT** перед выполнением команд.
...
Удаляет последнюю миграцию, вызывая `Down` метод последней миграции. Команда выполняет откат изменений кода, выполненных для последней миграции.

Параметры:

| Параметр                 | Short             | Описание                                                                     |
|:-----------------------|:------------------|:--------------------------------------------------------------------------------|
| <nobr>`--force`</nobr> | <nobr>`-f`</nobr> | Отменить последнюю миграцию и откатить изменения, которые были применены к базе данных во время последней миграции. |
