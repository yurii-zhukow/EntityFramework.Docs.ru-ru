...
* `DbContext` для EF Core можно настроить множеством способов. Информация о них приведена в статье [Настройка DbContext](xref:core/miscellaneous/configuring-dbcontext).
...
Узнайте больше о [настройке DbContext](xref:core/miscellaneous/configuring-dbcontext).
