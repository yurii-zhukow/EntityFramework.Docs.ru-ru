...
> [!TIP]
> Вы можете запустить и отладить весь код в этом документе, [загрузив пример кода из GitHub](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/ChangeTracking/ChangingFKsAndNavigations).
...
<!--
        ...
        Console.WriteLine(context.ChangeTracker.DebugView.LongView);
-->
[! код-CSharp[Relationship_fixup_2](../../../ samples / core / ChangeTracking / ChangingFKsAndNavigations / OptionalRelationshipsSamples.cs ? name = Relationship_fixup_2
...
> [!TIP]
> Каскадное удаление и удаление потерянных объектов тесно связаны друг с друга. В результате удаляются зависимые и дочерние сущности, когда связь с требуемым участником или родителем является серьезной. Для каскадного удаления это происходит потому, что сам участник или родитель удаляется. Для потерянных сущностей основная и родительская сущности по-прежнему существует, но больше не связана с зависимыми и дочерними сущностями.
