...
> [!TIP]  
> По внедрению этого нового шаблона. при обновлении приложений до версии 2.0 настоятельно рекомендуется необходима функций продукта, как Entity Framework Core миграции для работы. Типичная альтернатива — [реализовать *IDesignTimeDbContextFactory\<TContext >*](configuring-dbcontext.md#using-idesigntimedbcontextfactorytcontext).

2. Приложения, предназначенные для ASP.NET Core 2.0 можно использовать EF Core 2.0 без дополнительных зависимостей, кроме базы данных сторонних поставщиков. Тем не менее приложения, предназначенные для предыдущих версий ASP.NET Core нужно обновлять до Core ASP.NET 2.0 для использования EF Core 2.0. Подробнее об обновлении приложений ASP.NET Core 2.0 [ASP.NET основной документацией по этой теме](https://docs.microsoft.com/aspnet/core/migration/1x-to-2x/).
...
Коды событий для отправки сообщений на [ILogger](https://github.com/aspnet/Logging/blob/dev/src/Microsoft.Extensions.Logging.Abstractions/ILogger.cs) были изменены в версии 2.0. Идентификаторы событий являются уникальными в EF основного кода. Эти сообщения также выполнить стандартный шаблон структурированных ведения журнала, используемого, например MVC.

Категории ведения журнала также были изменены. Теперь хорошо известный набор категорий, доступных через [DbLoggerCategory](https://github.com/aspnet/EntityFramework/blob/dev/src/EFCore/DbLoggerCategory.cs).
...
EF Core 2.0 теперь будет построен другой [Тип IModel](https://github.com/aspnet/EntityFramework/blob/dev/src/EFCore/Metadata/IModel.cs) для каждого используемого разных поставщика. Это обычно прозрачно для приложения. Это добиться Упрощение интерфейсов API метаданных более низкого уровня, таким образом, что любой доступ к _основные понятия реляционных метаданных_ всегда устанавливается путем вызова `.Relational` вместо `.SqlServer`, `.Sqlite`и т. д. Например код 1.1.x следующим образом:
...
Вместо того чтобы использовать методы, такие как `ForSqlServerToTable`, методы расширения теперь доступны для записи условного кода на основании текущего поставщика используется. Например:
...
Глобальные без имени базы данных в памяти было удалено, а вместо этого должны иметь имена всех баз данных в памяти. Например:
