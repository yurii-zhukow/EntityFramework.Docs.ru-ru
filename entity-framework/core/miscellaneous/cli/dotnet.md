...
Целевой проект служит для добавления всех файлов (в некоторых случаях они удаляются из него). Проект по умолчанию проект в текущем каталоге, но можно изменить с помощью <nobr> **`--project`** </nobr> параметр.

Запускаемый проект эмулируется инструментами при выполнении кода проекта. Он также по умолчанию в проект в текущем каталоге, но можно изменить с помощью **`--startup-project`** параметр.
...
| -c | `--context <DBCONTEXT>`           | DbContext для использования.       |
| -p | `--project <PROJECT>`             | Проект для использования.         |
| -s | `--startup-project <PROJECT>`     | Запускаемый проект для использования. |
|    | `--framework <FRAMEWORK>`         | Целевая платформа.       |
|    | `--configuration <CONFIGURATION>` | Используемая конфигурация.   |
|    | `--runtime <IDENTIFIER>`          | Среда выполнения для использования.         |
...
|              |                                                                                              |
|:-------------|:---------------------------------------------------------------------------------------------|
| `<MIGRATION>` | Целевой объект миграции. Если значение равно 0, будут отменены все миграции. По умолчанию к последней миграции. |
...
| `<PROVIDER>`   | Используемый поставщик. (например, Microsoft.EntityFrameworkCore.SqlServer) |
...
| -c              | `--context <NAME>`                       | Имя DbContext.                                                                       |
|                 | `--context-dir <PATH>`                   | Поместите файл DbContext в каталог. Пути задаются относительно каталога проекта.             |
| -f              | `--force`                                 | Перезаписывайте существующие файлы.                                                                        |
| -o              | `--output-dir <PATH>`                    | Чтобы поместить файлы каталог. Пути задаются относительно каталога проекта.                      |
|                 | <nobr>`--schema <SCHEMA_NAME>...`</nobr> | Схемы таблиц для создания типов сущности для.                                              |
| -t              | `--table <TABLE_NAME>`...                | Для создания типов сущности для таблиц.                                                         |
|                 | `--use-database-names`                    | Используйте имена таблиц и столбцов непосредственно из базы данных.                                           |
...
|         |                                                               |
|:--------|:--------------------------------------------------------------|
| `<FROM>` | Начала миграции. По умолчанию равно 0 (исходной базы данных). |
