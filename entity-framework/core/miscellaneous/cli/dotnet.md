...
Команды включены в пакет SDK для .NET Core, но для включения команд, позволяющих `Microsoft.EntityFrameworkCore.Design` установить пакет. `dotnet ef`
...
* Измените файл проекта и добавьте `Microsoft.EntityFrameworkCore.Tools.DotNet` его `DotNetCliToolReference` в качестве элемента. Укажите последнюю версию версии 1. x, например: 1.1.6. См. пример файла проекта в конце этого раздела.
