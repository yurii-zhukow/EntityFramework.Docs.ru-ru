...
| <nobr>`-c`</nobr> | `--context <DBCONTEXT>`           | `DbContext` Класс для использования. Имя класса, только из цифр или полное с пространствами имен.  Если этот параметр опущен, найти класс контекста EF Core. Если имеется несколько классов контекста, этот параметр является обязательным.                                            |
