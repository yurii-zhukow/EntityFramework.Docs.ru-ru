...
Эти средства автоматически включаются в проект ASP.NET Core 2.1 +, так как пакет `Microsoft.EntityFrameworkCore.Tools` входит в состав [Microsoft. AspNetCore. app метапакет](/aspnet/core/fundamentals/metapackage-app).
...
* Измените *CSPROJ* -файл и добавьте строку, указывающую последнюю версию пакета [Microsoft. EntityFrameworkCore. Tools](https://www.nuget.org/packages/Microsoft.EntityFrameworkCore.Tools/) . Например, файл *CSPROJ* может содержать `ItemGroup`, который выглядит следующим образом:
...
| -Project \<строка >        | Целевой проект. Если этот параметр не указан, в качестве целевого проекта используется **проект по умолчанию** для **консоли диспетчера пакетов** .                                                                             |
| -Стартуппрожект \<строка > | Запускаемый проект. Если этот параметр не указан, в качестве целевого проекта используется **запускаемый проект** в **свойствах решения** .                                                                                 |
...
| Параметр                         | Описание                                                                                                             |
|:----------------------------------|:------------------------------------------------------------------------------------------------------------------------|
| <nobr>-Name \<строка ><nobr>       | Имя миграции. Этот параметр является позиционированным и является обязательным.                                              |
| <nobr>-OutputDir \<строка ></nobr> | Используемый каталог (и подпространство имен). Пути задаются относительно целевого каталога проекта. По умолчанию используется значение "миграции". |
...
Создает код для `DbContext` и типов сущностей для базы данных. Чтобы `Scaffold-DbContext` создать тип сущности, таблица базы данных должна иметь первичный ключ.

Параметры:

| Параметр                          | Описание                                                                                                                                                                                                                                                             |
|:-----------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <nobr>— Строка \<подключения ></nobr> | Строка подключения к базе данных. Для проектов ASP.NET Core 2. x значение может быть *Name =\<имя строки подключения >* . В этом случае имя берется из источников конфигурации, настроенных для проекта. Этот параметр является позиционированным и является обязательным. |
| <nobr>-Provider \<строка ></nobr>   | Используемый поставщик. Обычно это имя пакета NuGet, например: `Microsoft.EntityFrameworkCore.SqlServer`. Этот параметр является позиционированным и является обязательным.                                                                                           |
| -OutputDir \<строка >               | Каталог для размещения файлов. Пути задаются относительно каталога проекта.                                                                                                                                                                                             |
| -Контекстдир \<строка >              | Каталог, в который помещается файл `DbContext`. Пути задаются относительно каталога проекта.                                                                                                                                                                              |
| — Строка \<контекста >                 | Имя создаваемого класса `DbContext`.                                                                                                                                                                                                                          |
...
| Параметр                | Описание                                                                                                                                                                                                                |
|:-------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *— Из* строки \<        | Начало миграции. Миграция может быть идентифицирована по имени или по ИДЕНТИФИКАТОРу. Число 0 — это особый случай, который означает *перед первой миграцией*. Значение по умолчанию — 0.                                                              |
| *—* \<строки >          | Завершение миграции. По умолчанию используется последняя миграция.                                                                                                                                                                      |
| <nobr>-Идемпотентными</nobr> | Создание скрипта, который может использоваться в базе данных при любой миграции.                                                                                                                                                         |
| -Output \<строка >        | Файл, в который записывается результат. Если этот параметр не указан, файл создается с именем, созданным в той же папке, что и файлы среды выполнения приложения, например: */obj/Debug/netcoreapp2.1/ghbkztfz.SQL/* . |
