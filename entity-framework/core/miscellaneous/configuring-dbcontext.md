...
Однако любой код, который явным образом выполняет несколько потоков в параллельном режиме, должен обеспечить одновременное обращение к `DbContext`ным экземплярам.
