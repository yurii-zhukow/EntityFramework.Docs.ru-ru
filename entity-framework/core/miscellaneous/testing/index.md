...
Преимущество состоит в том, что переход между базами данных может быть относительно простым.
...
Кроме того, в SQL Server есть компонент под названием [LocalDB](/sql/database-engine/configure-windows/sql-server-express-localdb?view=sql-server-ver15).
...
Однако LocalDB имеет ряд ограничений:
...
Однако последний вариант может подойти некоторым разработчикам с более слабыми машинами.  
...
Вместо этого при модульном тестировании решений, применяющих DbContext, мы используем выполняющуюся в памяти базу данных.
В этом случае ее использование целесообразно, так как тест не зависит от работы базы.
Только не прибегайте к этому варианту для тестирования реальных запросов и операций обновления.   

Конкретные рекомендации для EF Core по модульному тестированию с использованием выполняющейся в памяти базы данных см. в статье [Тестирование с помощью InMemory](xref:core/miscellaneous/testing/in-memory).
