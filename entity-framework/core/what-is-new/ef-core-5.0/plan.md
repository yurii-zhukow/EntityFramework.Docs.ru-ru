...
Функция "многие ко многим" является [наиболее востребованной](https://github.com/aspnet/EntityFrameworkCore/issues/1368) (около 407 голосов) в журнале невыполненной работы GitHub.

Все вопросы о поддержке связей "многие ко многим" отслеживаются по тегу [#10508](https://github.com/aspnet/EntityFrameworkCore/issues/10508). Эту поддержку можно разбить на три основные области:

* Свойства навигации с пропуском. Они позволяют использовать модель для запросов и т. п. без ссылки на базовую сущность таблицы соединений. ([#19003](https://github.com/aspnet/EntityFrameworkCore/issues/19003))
* Типы сущностей контейнера свойств. Они позволяют использовать стандартный тип CLR (например, `Dictionary`) для экземпляров сущностей таким образом, чтобы для каждого типа сущности не требовался явный тип CLR. (Stretch для 5.0: [#9914](https://github.com/aspnet/EntityFrameworkCore/issues/9914).)
* Средства для простоты настройки связей "многие ко многим". (Stretch для 5.0.)
