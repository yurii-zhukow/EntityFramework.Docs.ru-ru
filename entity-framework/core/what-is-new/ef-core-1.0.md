...
Это атрибуты, которые можно добавлять в классы и (или) свойства сущности, и которые будут влиять на модели EF (например, атрибут [required] сообщит платформе EF, что соответствующее свойство является обязательным).
...
Запросы выполняются быстрее, если контекст не отслеживает изменения экземпляров сущностей (т. е. результаты доступны только для чтения).
