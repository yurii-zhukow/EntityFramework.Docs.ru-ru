...
> [!NOTE]
> Старайтесь не использовать <xref:System.Linq.Enumerable.ToList%2A> или, <xref:System.Linq.Enumerable.ToArray%2A> Если планируется использовать другой оператор LINQ в результате. это приведет к ненужному буферу всех результатов в памяти. Используйте вместо этого <xref:System.Linq.Enumerable.AsEnumerable%2A>.
