...
---
# <a name="required-and-optional-properties"></a>Обязательные и дополнительные свойства

Свойство считается необязательным, если оно является допустимым для того, чтобы оно содержало `null`. Если `null` не является допустимым значением для присвоения свойству, оно считается обязательным свойством.
