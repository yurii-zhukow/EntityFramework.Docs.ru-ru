...
> [!NOTE]
> До EF Core 3,0 метод `WithOwner()` не существовал, поэтому этот вызов следует удалить.
...
По умолчанию EF Core будет наименовать столбцы базы данных для свойств собственного типа сущности после шаблона _Navigation_OwnedEntityProperty_. Поэтому свойства `StreetAddress` будут отображаться в таблице Orders с именами ' ShippingAddress_Street ' и ' ShippingAddress_City '.
...
Обратите внимание на `WithOwner` вызов, используемый для настройки свойства навигации, указывающего на владельца.

Результат можно получить с помощью `OwnedAttribute` как в `OrderDetails`, так и в `StreetAdress`.
