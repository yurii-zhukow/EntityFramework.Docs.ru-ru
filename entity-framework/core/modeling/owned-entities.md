...
Объект, к которому типы никогда не включаются ядром EF в модели по соглашению, которым владеет. Можно использовать `OwnsOne` метод в `OnModelCreating` или указать для типа `OwnedAttrbibute` (новые возможности EF Core 2.1) чтобы настроить тип как тип владельца.
